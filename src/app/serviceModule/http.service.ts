...
export class HttpService{
	...
	*/
	getObject<T>(clazz: { new (): T }, url: string): Observable<HttpResult<T>>{
		...
		return this.httpClient.get(Constants.baseUrl + url + '?' + 'user_email=' + userEmail + '&user_token=' + userToken).pipe(map(data => {
	      ...
	      return new HttpResult<T>(resultData, null);
	    	...
	    	return of(new HttpResult<T>(null, [
	...
	*/
	...
	private httpRequest(url: string, reququestBody: string, method: RequestMethod): Observable<HttpResult<any>>{
 	    ...
 	    return this.http.put(Constants.baseUrl + url, reququestBody, options).pipe(map(data => {
 
 	      return new HttpResult<any>(data, null);
 
 	    }), catchError((response: Response) => {
 
	    	return of(new HttpResult<HttpMessage>(null, [
