...
export class EventService{
	...
	*/
	getEvent(eventId: number, useMockData: boolean = false): Observable<HttpResult<Event>>{
		...
		if(useMockData){
			...
			return of(new HttpResult<Event>(this.mockEventData, null)).pipe(map(result => {
		...
		if(!UtilityFunctions.isNullOrUndefined(this._event) && this._event.id === eventId){
			return of(new HttpResult<Event>(this._event, null));
		}

		//Grab the event information from the API
		return this.httpService.getObject<Event>(Event, "events/" + eventId + ".json").pipe(map(result => {
	...
	*/
	getEventParticipants(eventId: number, useMockData: boolean = false): Observable<HttpResult<Participant[]>>{
		...
		if(!UtilityFunctions.isNullOrUndefined(this._event) && this._event.id === eventId){
			return of(new HttpResult<Participant[]>(this._event.participants));
		}

		//Test code
		if(useMockData){
			return of(new HttpResult<Participant[]>(this.mockParticipantData));
		}
	...
	getEventName(eventId: number, useMockData: boolean = false): Observable<HttpResult<string>>{
		...
		//If cached data exists, just return that
		if(!UtilityFunctions.isNullOrUndefined(this._event) && this._event.id === eventId){
			return of(new HttpResult<string>(this._event.name));
		}

		//Test code
		if(useMockData){
			return of(new HttpResult<string>("Event #" + eventId));
		}
		...
		return this.apollo.query({
	    	...
	    	return of(new HttpResult<string>(null, [
	...
	inviteUsersToEvent(invitedUsers: User[], eventId: number): Observable<HttpResult<Participant[]>>{
	    ...
	    return this.httpService.put("events/" + eventId, JSON.stringify(putParams)).pipe(map(result => {
