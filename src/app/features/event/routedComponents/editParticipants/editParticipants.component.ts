...
export class EditParticipantsComponent extends EventSubPageComponent{
	...
	****************/
	...
	ngOnInit() {
		...
		this.inviteInput.valueChanges.pipe(startWith(null), map(val => {
		...
		this.route.parent.paramMap.pipe(switchMap((params: ParamMap) => {
			...
			let _zippedObservable: Observable<{ userResult: HttpResult<User[]>, participantResult: HttpResult<Participant[]>, getNameResult: HttpResult<string> }> = zip(this.userService.getAllSystemUsers(USE_MOCK_DATA), this.eventService.getEventParticipants(+params.get('eventId'), USE_MOCK_DATA), this.eventService.getEventName(+params.get('eventId'), USE_MOCK_DATA), (userResult, participantResult, getNameResult) => {
